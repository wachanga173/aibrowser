document.addEventListener('DOMContentLoaded', () => {
  ...
  const agentHighlightEnabled = document.getElementById('agentHighlightEnabled');
  const agentSemanticEnabled = document.getElementById('agentSemanticEnabled');
  const tabAutoSuspend = document.getElementById('tabAutoSuspend');

  const optionsMemorySummary = document.getElementById('optionsMemorySummary');
  const optionsOptimizeBtn = document.getElementById('optionsOptimizeBtn');
    ...
    btnTestVector.addEventListener('click', () => {
      ...
      chrome.runtime.sendMessage({ type: 'VECTOR_SEARCH', queryEmbedding: [0.9, 0.1, 0.0] }, (resp) => {
        if (resp && resp.ranked_topics) {
          nativeHostOutput.textContent = `Vector Search Topics:\n${resp.ranked_topics.map(t => `• ${t[0]} (Rank Score: ${t[1].toFixed(2)})`).join('\n')}`;
        } else {
