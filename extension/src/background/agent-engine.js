...
export class BrowserAIAgent {
  async processQuery(userPrompt, pageData) {
  ...
  async tryGeminiNano(userPrompt, pageData) {
    try {
  ...
  executeLocalNLP(prompt, intent, page) {
    switch (intent) {
  ...
  generateConversationalResponse(prompt, page) {
  ...
  generateSimplifiedExplanation(page) {
    const sentences = splitIntoSentences(page.rawText);
    const ranked = computeTfIdfRankings(sentences);
    ranked.sort((a, b) => b.score - a.score);
  ...
  extractTopSentences(rawText, count) {
    const sentences = splitIntoSentences(rawText);
    const ranked = computeTfIdfRankings(sentences);
    ranked.sort((a, b) => b.score - a.score);
